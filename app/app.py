...
async def upload_file(
    ...
    try:
        ...
        if upload_result.response_metadata.http_status_code == 200:
            ...
            raise HTTPException(status_code=500, detail=str(upload_result.get("error")))
...
async def get_feed(session: AsyncSession = Depends(get_async_session), user: User = Depends(current_active_user)):
    ...
    posts = [post[0] for post in posts.fetchall()]
    ...
    posts_data = []
    for post in posts:
        posts_data.append(PostResponse(
            id=str(post.id),
